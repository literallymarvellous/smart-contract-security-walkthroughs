...
contract LendingPool is OwnableUpgradeable, ERC721Upgradeable, Clone {
    ...
    event ReducedDailyBorrows(
        uint256 currentDailyBorrows,
    ...
    function initialize(
        ...
        maxVariableInterestPerEthPerSecond = interests
    ...
    function addDailyBorrows(uint216 toAdd) internal {
        ...
        if (toReduce > currentDailyBorrows) {
            ...
            currentDailyBorrows =
                uint216(currentDailyBorrows - toReduce) +
                toAdd;
        }
    ...
    function getLoanId(
        ...
        return
            uint256(keccak256(abi.encode(nftId, interest, startTime, price)));
    }
    ...
    function calculateInterest(uint256 priceOfNextItems)
        internal
        view
    ...
    function _burnWithoutBalanceChanges(uint256 tokenId, address owner)
        internal
    {
        // Clear approvals
        ...
        returns (uint256)
    {
        ...
        if (sinceLoanStart > maxLoanLength) {
            interest +=
                ((sinceLoanStart - maxLoanLength) * borrowed) /
                (1 days);
        }
    ...
    function repay(Loan[] calldata loansToRepay, address from)
        external
        payable
    {
    ...
    function doEffectiveAltruism(Loan calldata loan, address to) external {
        ...
        require(
            block.timestamp > (loan.startTime + maxLoanLength),
            "not expired"
        );
        ...
        )
    {
        ...
        if (block.timestamp > deadline) {
            lateFees =
                ((block.timestamp - deadline) * loan.borrowed) /
                (1 days);
        } else {
            lateFees = 0;
        }
    ...
    function currentAnnualInterest(uint256 priceOfNextItem)
        external
        view
        returns (uint256 interest)
    {
        ...
        )
    {
        ...
        maxInstantBorrow = Math.min(
            address(this).balance,
            maxDailyBorrows - dailyBorrows
        );
    }
    ...
    function changeInterest(
        uint256 _maxInterestPerEthPerSecond,
        uint256 _minimumInterest
    ) external onlyOwner {
